...
3. Git, Python 및 uv 오프라인 설치파일을 같은 폴더에 유지하면 다운로드 없이 설치됩니다.
   파일이 없으면 아래 공식 배포처에서 자동 다운로드한 뒤 SHA256을 검증합니다.
...
2. install.bat 을 더블클릭
   ...
   - 일반 권한에서 시작하면 앱과 시스템 구성요소를 설치하기 전에 관리자 권한 창이 한 번 뜹니다.
   - UAC를 승인하면 SAMWOO-ORCA와 시스템 구성요소가 같은 관리자 단계에서 설치됩니다.
3. 설치가 끝나면 SAMWOO-ORCA가 자동 실행됩니다.

설치되는 것:
- SAMWOO-ORCA 앱 (전체 사용자용으로 설치)
...
네트워크 설치:
- Git, Python, uv 또는 Tailscale 설치파일이 폴더에 없으면 인터넷 연결이 필요합니다.
- 다운로드 또는 관리자 단계가 실패하면 설치 로그 `%TEMP%\samwoo-orca-install.log`의
  설치 요약에 FAIL로 표시되며, 필수 구성요소가 모두 준비되기 전에는 앱을 자동 실행하지 않습니다.

업데이트 주의:
- 전체 사용자용 설치 파일은 Windows 정책상 설치·업데이트 때 UAC 승인을 요구할 수 있습니다.
- 설치는 마법사 없이 진행 표시만 보여주고 완료 후 자동으로 닫히며 앱을 다시 실행합니다.
