...
- 남은 단계: Windows package/sign CI와 설치본 GUI 첨부·native save 실측. 로컬 package는 기존 `node-pty` 재빌드에 필요한 Visual Studio Build Tools가 없어 중단됐고, 재빌드를 생략한 리소스 스모크도 electron-builder의 pnpm dependency-tree 출력 파싱 단계에서 중단됐다. 기능 소스나 frozen worker 실패로 보지 않되 정식 패키지 성공으로도 간주하지 않는다.
