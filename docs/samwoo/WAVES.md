...
| W6a | 프로필 멤버 디렉터리·메신저 실명 표시 | v1.4.186 공개 완료·VPS/GUI 실측 대기 | f4d4df553, 0262f692d, run 31318757576 |
| W6b | 허브 2단계 — 담당자·마감일·작업 항목 | v1.4.186 공개 완료·VPS/GUI 실측 대기 | 30e8ef9b8, 086c4a4fd, 0262f692d, run 31318757576 |
...
| W11 | 로그인 식별자 정규화·릴리스 UI 정리 | ✅ v1.4.186 공개 완료·VPS/GUI 실측 대기 | 0262f692d, 5fc6b6224, edff354cf, run 31318757576 |
...
### W6a — 프로필 멤버 디렉터리·메신저 실명 표시 (login 수정 반영 대기)
...
- 검증: Python 서버 전체 72개, focused Vitest 16개, TypeScript 3종, oxlint·React 규칙, max-lines, i18n 게이트 통과. VPS `role-map.csv` fallback은 정상이며, 메일 주소 login 불일치는 W11에서 수정했다. 핫 리로드·2계정 실명 GUI 실측은 `1.4.186`과 VPS 반영 뒤 진행한다.

### W6b — 허브 2단계 (login 수정 반영 대기)
...
- 서버 라우트 배포와 401 라우팅 확인은 완료됐다. 1.4.184 GUI 실패 원인은 메일 주소 login과 디렉터리의 짧은 아이디 불일치로 확정했으며 W11에서 서버·앱 정규화와 기존 DB 마이그레이션을 구현했다. `1.4.186`과 VPS 반영 뒤 담당자·마감일·작업 항목을 다시 실측한다.
...
### W11 — 로그인 식별자 정규화 (v1.4.186 공개 완료·운영 실측 대기)
...
- 남은 단계: VPS 서버 모듈 재배포·재시작, 앱 업데이트 후 재로그인, 2계정 GUI 실측.
