...
| W6a | 프로필 멤버 디렉터리·메신저 실명 표시 | 서버 배포 완료·GUI 실명 확인 필요 | f4d4df553 |
| W6b | 허브 2단계 — 담당자·마감일·작업 항목 | 서버 배포 완료·GUI 실패 원인 확인 필요 | 30e8ef9b8, 086c4a4fd |
...
- 남은 실측: 2계정 SSE·프레즌스, 메신저 실명, 담당자·마감일·작업 항목, 메일 첨부와 봇 메시지 조회. 1.4.184 실사용에서 온라인 수는 보이나 실명이 상시 보이지 않고 담당자·마감일 조작이 실패한다는 보고가 있어 원인 확인이 필요하다.
...
### W6a — 프로필 멤버 디렉터리·메신저 실명 표시 (서버 배포 완료, GUI 확인 필요)
...
- 검증: Python 서버 전체 72개, focused Vitest 16개, TypeScript 3종, oxlint·React 규칙, max-lines, i18n 게이트 통과. VPS `role-map.csv` fallback 배포는 확인됐고 핫 리로드·2계정 실명 GUI 실측은 남아 있다.

### W6b — 허브 2단계 (서버 배포 완료, GUI 실패 원인 확인 필요)
...
- 서버 라우트 배포와 401 라우팅 확인은 완료됐다. 1.4.184 GUI에서 담당자·마감일 조작 실패가 보고되어 인증 세션·권한·응답 본문을 포함한 실계정 원인 확인이 필요하다.
