...
| 기본 브랜치      | `main`                                        |
| 최신 공개 릴리스 | `v1.4.216` — 2026-08-19 운영 확인             |
...
| 업데이트 피드   | `github.com/samwooax/samwoo-orca/releases` / HTTPS | 공개 릴리스 업데이트                     | `v1.4.216` 공개·latest 확인 (2026-08-19)                                                  |
...
- Python 3.13, 문서 engine과 고정 버전 LibreOffice는 Windows 설치본에 포함하며 사용자는 Python, pip, LibreOffice 또는 별도 package를 설치하지 않는다. capability는 main의 bundle integrity·engine probe가 성공할 때만 모델에 제공한다.
...
- 스캔 PDF OCR, 기존 PDF 본문의 무손실 임의 치환, 매크로·ActiveX·OLE·전자서명 보존은 지원하지 않고 원본을 변경하지 않은 채 명시적으로 실패한다. 범용 Excel Artifact `renderPreview`는 계속 비활성이며, 아래 exact `ai_center` ACP 시각 읽기만 별도 신뢰 경계로 제공한다.

2026-08-18 구현 완료된 ACP 경로는 기존 구조화 명령 브리지를 대체하지 않는 별도 rollout이다. 정확한 `ai_center` 프로필에서 Store가 승인한 local worktree/folder workspace를 열면 개발·패키지 실행 모두 ACP 파일과 terminal/process 기능을 자동으로 광고한다. 별도 환경변수나 실행 옵션은 필요하지 않다. 다른 프로필, SSH/Runtime workspace, 승인되지 않았거나 해석되지 않은 workspace는 기존 경로를 유지한다.
...
| 08-18 | exact `ai_center` ACP Office 시각 읽기                                 | Windows 설치본에 고정 LibreOffice를 포함하고 XLSX/PPTX를 bounded image로 렌더해 같은 vision turn에 전달한다. 범용 Excel Artifact render capability와 다른 profile은 바꾸지 않는다 |
...
| `v1.4.215` | 공개 유지                | bundled LibreOffice XLSX/PPTX 시각 미리보기. Actions run `32203907871`, 설치기 978,924,520바이트·SHA-256 `3667c22f…64ad`·서명/자산 검증 후 공개 latest·tag 확인                                          |
| `v1.4.216` | **공개 — 최신**          | exact `ai_center` ACP bundled Excel Artifact 복구, HTML·UTF-8 첨부와 Office 원본 해시·진행 상태 보완. Actions run `32210279875`, 설치기 978,921,032바이트·SHA-256 `d191d278…20cc36`·서명/자산 검증 후 공개 |
