...
> 기준: 2026-08-19, Git commit `1074bd27e`, `package.json` 버전 `1.4.216`
>
...
| Hermes document worker     | 사용자 PC의 main-owned frozen Python child process | 대형 XLSX 스트리밍 추출, XLSX/PPTX/PDF 생성·수정·검증, LibreOffice 시각 렌더 | Windows x64 설치본에 고정 engine·LibreOffice와 함께 포함하며 main만 실행·취소·파일 commit 권한을 가짐. SSH/Runtime에서는 실행하지 않음 |
...
- native file picker는 알려진 문서 확장자와 `All files`를 노출하고, 예약된 binary 확장자를 제외한 regular·non-symlink·96KB 이하 strict UTF-8·NUL-free 파일을 text attachment로 판별한다. 64MiB 이하 PDF/XLSX/PPTX/PNG/JPEG는 signature를 확인하며 renderer/Base64에 싣지 않고 Electron main의 private artifact store로 복사·해시한 뒤 opaque ID만 반환한다.
...
- Hermes의 ACP Python SDK는 추가 keyword를 wire `_meta`로 감싸므로 bridge는 `samwoo` namespace를 직접 넘긴다. 없는 파일 read는 native path 대신 `file does not exist`를 반환하고 같은 turn의 신규 write를 허용한다. Office preview는 request/response 양쪽의 exact versioned `_meta`가 일치할 때만 sentinel을 `_multimodal` 이미지 결과로 바꾼다.
- Electron main executor는 canonical project-root jail, `/workspace` POSIX path, traversal·backslash·ADS·Windows device name·symlink/hardlink·`.git` write 거부, UTF-8 regular file 및 512KiB 상한을 적용한다. `.xlsx`/`.pptx` read만 64MiB까지 별도 worker에 넘기고 원본 SHA-256을 인증된 multimodal summary에 포함한다. 기존 파일 write는 같은 turn의 prior-read SHA-256을 요구하고 project queue 안에서 재검증·backup·temp rename을 수행한다. Backup은 app userData 아래 private directory에 원본 bytes로 보관하며 file당 20개, 30일, project당 256MiB, 전체 512MiB 상한으로 정리한다.
...
- PDF.js와 OOXML 읽기·번역은 30초 timeout·memory limit이 있는 Node worker thread에서 실행한다. PDF.js는 PDF 요청에서만 lazy-load하고, Node용 DOMMatrix/ImageData/Path2D를 제공하는 플랫폼별 `@napi-rs/canvas`와 `pdf.worker.mjs`를 packaged resource 경계에서 확인한다. 일반 생성·편집은 Python 3.13과 고정된 openpyxl/XlsxWriter/python-pptx/pypdf/reportlab을 PyInstaller one-folder 실행파일로 빌드한다. Windows build는 공식 LibreOffice 26.2.5 x64 MSI를 고정 size/SHA-256으로 검증해 worker 아래에 administrative extract하고 license/source notice와 함께 같은 recursive manifest 및 `Resources/hermes-excel-artifact-worker` 경계에 포함한다. Electron→Python JSONL은 환경과 host 양쪽에서 UTF-8로 고정하며, main은 사용자 파일명과 분리된 ASCII staging 이름을 생성하고 성공·실패 모두 `finally`에서 제거한다. 사용자의 Python·pip·Office package를 사용하지 않는다.
...
- Electron main은 worker bundle manifest와 실제 engine metadata를 probe한 경우에만 Excel Artifact v1의 `create`/`modify`/`validate`/`cancel` capability를 trusted instruction에 넣는다. Team Chat을 여는 IPC가 이 process-wide cached probe를 background에서 미리 시작해 첫 메시지의 cold bundle 검증 대기를 줄인다. `<orca_excel_artifact>`는 local file/document/command envelope와 하나의 union으로 parse하고, capability가 없거나 WSL/SSH/Runtime workspace이면 실행하지 않는다. exact `ai_center` native-local ACP에서는 file/terminal capability와 이 Excel capability를 함께 광고하며, 모델의 Excel envelope만 Electron outer tool loop로 전달해 같은 persistent ACP session에 결과를 돌려준다. 다른 Orca envelope는 ACP 경로에서 fail-closed하고, XLSX 작업을 사용자 Python·pip·직접 soffice 실행으로 우회하지 않는다. 범용 Excel Artifact render capability는 계속 비활성이지만 `.xlsx`/`.pptx` `read_file`은 별도 trusted-unsandboxed preview action을 사용한다. worker가 OOXML을 검증·staging한 뒤 LibreOffice→PDF→PDFium/Pillow로 최대 4개를 contact sheet화하고, raw image 700KiB·long side 1600px·render 120초·worker 160초 상한 안의 PNG/JPEG Base64만 main이 허용한다. ACP client는 cold manifest probe를 포함해 270초 안에 응답을 받고, 취소·timeout은 worker와 LibreOffice descendant tree를 정리한 뒤 완료한다. 대화 기록에는 최신 Office 이미지만 남기고 앞선 이미지는 active 여부와 무관하게 텍스트 요약으로 축약한다.
...
1. **시각 검증 격리 강화**: exact `ai_center` trusted-local preview는 구현됐지만 OS sandbox는 아니다. 범용 render capability는 Windows Job Object/no-network 격리와 preview artifact registry를 검증한 뒤에만 활성화한다.
...
| 높음     | Hermes ACP local terminal           | 정확한 `ai_center`로 제한하지만 명령별 승인 없이 OS 사용자 권한으로 즉시 실행하며 project 밖 파일·network에도 접근 가능 | 조직이 이 full-trust 정책을 유지할지 주기적으로 재검토하고, 필요하면 outbound policy·격리·감사 UI를 별도 추가          |
