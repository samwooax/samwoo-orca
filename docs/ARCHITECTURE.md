...
> 기준: 2026-08-18, Git commit `9602db864`, `package.json` 버전 `1.4.214`
>
...
- Electron main executor는 canonical project-root jail, `/workspace` POSIX path, traversal·backslash·ADS·Windows device name·symlink/hardlink·`.git` write 거부, UTF-8 regular file 및 512KiB 상한을 적용한다. `.xlsx`/`.pptx` read만 64MiB까지 별도 worker에 넘긴다. 기존 파일 write는 같은 turn의 prior-read SHA-256을 요구하고 project queue 안에서 재검증·backup·temp rename을 수행한다. Backup은 app userData 아래 private directory에 원본 bytes로 보관하며 file당 20개, 30일, project당 256MiB, 전체 512MiB 상한으로 정리한다.
