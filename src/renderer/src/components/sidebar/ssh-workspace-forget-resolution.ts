 ...
 * workspace from SAMWOO-ORCA because the SSH target is gone entirely.
 */
export type SshWorkspaceForgetResolution =
  ...
  // impossible, so forget-from-SAMWOO-ORCA is the only path.
...
export function resolveSshWorkspaceForget(args: {
  ...
  // the only escape is to forget it from SAMWOO-ORCA.
  if (!isConfigured) {
