...
const RemoveFolderDialog = React.memo(function RemoveFolderDialog() {
  ...
  const description = sshHostLabel
    ? translate(
        ...
        'This only removes {{name}} from SAMWOO-ORCA. Its files stay on {{host}} — re-add that SSH host to recover it.',
        { name: NAME_TOKEN, host: sshHostLabel }
      )
    : translate(
        'auto.components.sidebar.RemoveFolderDialog.removeDescriptionLocal',
        'This only removes {{name}} from SAMWOO-ORCA. It is still on your disk.',
        { name: NAME_TOKEN }
      )
