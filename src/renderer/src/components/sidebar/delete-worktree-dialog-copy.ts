  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from SAMWOO-ORCA. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from SAMWOO-ORCA. Git worktrees will also be removed from git and disk; folder workspaces will only remove the SAMWOO-ORCA workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from SAMWOO-ORCA. The project folder on disk will not be deleted.'
