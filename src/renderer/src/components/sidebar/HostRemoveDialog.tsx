...
export function HostRemoveDialog({
  ...
  // error context owned by the SAMWOO-ORCA servers settings pane, so we deep-link
  ...
  const description =
    target.kind === 'runtime'
      ? translate(
          ...
          'This opens the SAMWOO-ORCA servers settings where you can remove this server.'
        )
      : hasWorkspaces
        ? translate(
            'auto.components.sidebar.HostRemoveDialog.hostHasWorkspacesDefault',
            'Removes {{value0}} and its credentials from this computer. Its {{value1}} stay in SAMWOO-ORCA — remote files are not touched.',
  ...
  // only forget SAMWOO-ORCA's records (offline/ghost host).
  const deleteOptionLabel = isConnected
    ...
    : translate(
        ...
        'Also remove these {{value0}} from SAMWOO-ORCA',
                    ...
                    <span className="mt-0.5 block text-muted-foreground">
                      {isConnected
                        ...
                        : translate(
                            ...
                            'Clears them from SAMWOO-ORCA only. Remote files, worktrees, and branches are left untouched.'
                          )}
                    </span>
                  </span>
                </div>
