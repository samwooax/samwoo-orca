...
// reads the clipboard with navigator.clipboard.readText(). SAMWOO-ORCA runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through SAMWOO-ORCA's
 ...
 * bridge); otherwise performs the paste through SAMWOO-ORCA's IPC clipboard read and
